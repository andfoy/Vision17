...
cdef inline void _compute_means(double[:, :] data,
                                int[:] assign,
                                double[:, :] means,
                                int[:] counts):
    """
...
cpdef tuple kmeans(double[:, :] data, np.npy_intp k,
    ...
    cdef:
        ...
        np.npy_intp nfeat = data.shape[1]
        dtype = data.dtype
        double[:] mindist = np.empty(ndata, dtype=dtype)
        int[:] counts = np.empty(k, dtype=int)
        # Allocate space for the assignment indices, distance matrix, the means.
        double[:, :] dists = np.empty((ndata, k), dtype=dtype)
        double[:] m_sqnorm = np.empty(k, dtype=dtype)
        double[:, :] means
    ...
    if init is not None:
        ...
        assign = rng.random_integers(0, k - 1, size=ndata).astype(int)
